...
/*! \file document.h */
#include <SDL3/SDL.h>
#define RAPIDJSON_ASSERT(x) SDL_assert(x)
...
template <typename Encoding, typename Allocator = RAPIDJSON_DEFAULT_ALLOCATOR> class GenericValue {
	...
	*/
	...
	/*! \note If the value is 64-bit integer type, it may lose precision. Use \c IsLosslessDouble() to check whether
	 * the converison is lossless.
	 */
	...
	/*! \note If the value is 64-bit integer type, it may lose precision. Use \c IsLosslessFloat() to check whether
	 * the converison is lossless.
	 */
	...
	*/
	...
	/*!
	    \tparam T Either \c bool, \c int, \c unsigned, \c int64_t, \c uint64_t, \c double, \c float, \c const \c
	   char*, \c std::basic_string<Ch>
	*/
	...
	*/
		...
		switch (GetType()) {
			...
			case kObjectType:
				...
				for (ConstMemberIterator m = MemberBegin(); m != MemberEnd(); ++m) {
					RAPIDJSON_ASSERT(m->name.IsString()); // User may change the type of name by
									      // MemberIterator.
	...
	enum {
		...
		kNullFlag = kNullType,
		// These casts are added to suppress the warning on MSVC about bitwise operations between enums of
		// different types.
	...
	static RAPIDJSON_FORCEINLINE Map*& GetMap(Member* members) {
		...
		return *reinterpret_cast<Map**>(reinterpret_cast<uintptr_t>(members) -
						RAPIDJSON_ALIGN(sizeof(SizeType)) - RAPIDJSON_ALIGN(sizeof(Map*)));
	}
...
class GenericDocument : public GenericValue<Encoding, Allocator> {
	...
	*/
	...
	GenericDocument& Parse(const std::basic_string<typename SourceEncoding::Ch>& str) {
		// c_str() is constant complexity according to standard. Should be faster than Parse(const char*,
		// size_t)
...
inline rapidjson::Value fromVector2f(const Eigen::Vector2f& vec, rapidjson::Document::AllocatorType& allocator) {
...
inline rapidjson::Value fromVector2i(const Eigen::Vector2i& vec, rapidjson::Document::AllocatorType& allocator) {
	rapidjson::Value jsonValue(rapidjson::kArrayType);
	jsonValue.PushBack(vec.x(), allocator);
	jsonValue.PushBack(vec.y(), allocator);
	return jsonValue;
